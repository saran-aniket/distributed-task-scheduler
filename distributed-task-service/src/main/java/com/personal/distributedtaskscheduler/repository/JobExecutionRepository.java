...
public interface JobExecutionRepository extends JpaRepository<JobExecution, UUID> {
    ...
    @Query("""
            ...
            set je.status = :claimedStatus,
                je.claimedByNode = :nodeId
            where je.id = :executionId
              and je.status = :pendingStatus
            """)
    int claimPendingExecution(
            @Param("executionId") UUID executionId,
            @Param("pendingStatus") JobExecutionStatus pendingStatus,
            @Param("claimedStatus") JobExecutionStatus claimedStatus,
